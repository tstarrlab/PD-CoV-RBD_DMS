...
dt[,c("Kd_APN","Kd_SE_APN","response_APN","baseline_APN","nMSR_APN") :=
...
We successfully generated *K*<sub>D</sub> estimates for `r sum(!is.na(dt[library=="pool1A",Kd_APN]))` of our pool1A barcodes (`r round(sum(!is.na(dt[library=="pool1A",Kd_APN]))/nrow(dt[library=="pool1A",])*100,digits=2)`%), and `r sum(!is.na(dt[library=="pool2A",Kd_APN]))` of our pool2A barcodes (`r round(sum(!is.na(dt[library=="pool2A",Kd_APN]))/nrow(dt[library=="pool2A",])*100,digits=2)`%). 
...
par(mfrow=c(2,2))
hist(log10(dt[library=="pool1A" & !is.na(Kd_APN),TiteSeq_avgcount]+0.5),breaks=20,xlim=c(0,5),main="pool1A",col="gray50",xlab="average cell count across concentration samples")
hist(log10(dt[library=="pool1A" & is.na(Kd_APN),TiteSeq_avgcount]+0.5),breaks=20,add=T,col="red")

hist(log10(dt[library=="pool2A" & !is.na(Kd_APN),TiteSeq_avgcount]+0.5),breaks=20,xlim=c(0,5),main="pool2A",col="gray50",xlab="average cell count across concentration samples")
hist(log10(dt[library=="pool2A" & is.na(Kd_APN),TiteSeq_avgcount]+0.5),breaks=20,add=T,col="red")

hist(dt[library=="pool1A" & !is.na(Kd_APN),TiteSeq_min_cell_filtered],breaks=5,main="pool1A",col="gray50",xlab="number of sample concentrations below cutoff cell number",xlim=c(0,10))
hist(dt[library=="pool1A" & is.na(Kd_APN),TiteSeq_min_cell_filtered],breaks=16,add=T,col="red")

hist(dt[library=="pool2A" & !is.na(Kd_APN),TiteSeq_min_cell_filtered],breaks=5,main="pool2A",col="gray50",xlab="number of sample concentrations below cutoff cell number",xlim=c(0,10))
hist(dt[library=="pool2A" & is.na(Kd_APN),TiteSeq_min_cell_filtered],breaks=16,add=T,col="red")
```
...
plot.titration <- function(row,output.text=F){
  ...
  if(output.text==T){ #for troubleshooting and interactive work, output some info from the counts table for the given row
    vars <- c("library","barcode","target","variant_class","aa_substitutions","TiteSeq_avgcount","TiteSeq_min_cell_filtered","Kd_APN","Kd_SE_APN","baseline_APN","response_APN","nMSR_APN")
    return(dt[row,..vars])
  }
}
```
...
par(mfrow=c(3,1))
hist(log10(dt[target=="Wuhan_Hu_1",Kd_APN]),col="gray40",breaks=60,xlab="log10(KD), APN (M)",main="Wuhan_Hu_1",xlim=c(-13,-5))
hist(log10(dt[target=="Wuhan_Hu_1" & variant_class %in% (c("synonymous","wildtype")),Kd_APN]),col="#92278F",add=T,breaks=60)
hist(log10(dt[target=="BA1",Kd_APN]),col="gray40",breaks=60,xlab="log10(KD), APN (M)",main="BA1",xlim=c(-13,-5))
hist(log10(dt[target=="BA1" & variant_class %in% (c("synonymous","wildtype")),Kd_APN]),col="#92278F",add=T,breaks=60)
hist(log10(dt[target=="BA2",Kd_APN]),col="gray40",breaks=60,xlab="log10(KD), APN (M)",main="BA2",xlim=c(-13,-5))
hist(log10(dt[target=="BA2" & variant_class %in% (c("synonymous","wildtype")),Kd_APN]),col="#92278F",add=T,breaks=60)
#save pdf
...
dt[variant_class == "stop",c("Kd_APN","Kd_SE_APN","response_APN","baseline_APN","nMSR_APN") := list(as.numeric(NA),as.numeric(NA),as.numeric(NA),as.numeric(NA),as.numeric(NA))]
```
...
par(mfrow=c(2,2))
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 9e-6)[1])
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 9e-6)[2])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 9e-6)[1])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 9e-6)[2])
```
...
par(mfrow=c(2,2))
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 1e-6 & dt$Kd_APN < 1.2e-6)[1])
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 1e-6 & dt$Kd_APN < 1.2e-6)[2])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 1e-6 & dt$Kd_APN < 1.2e-6)[1])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 1e-6 & dt$Kd_APN < 1.2e-6)[2])
```
...
par(mfrow=c(2,2))
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 1e-7 & dt$Kd_APN < 1.2e-7)[1])
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 1e-7 & dt$Kd_APN < 1.2e-7)[2])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 1e-7 & dt$Kd_APN < 1.2e-7)[1])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 1e-7 & dt$Kd_APN < 1.2e-7)[2])
```
...
par(mfrow=c(2,2))
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 1e-8 & dt$Kd_APN < 1.2e-8)[1])
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 1e-8 & dt$Kd_APN < 1.2e-8)[2])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 1e-8 & dt$Kd_APN < 1.2e-8)[1])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 1e-8 & dt$Kd_APN < 1.2e-8)[2])
```
...
par(mfrow=c(2,2))
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 1e-9 & dt$Kd_APN < 1.2e-9)[1])
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 1e-9 & dt$Kd_APN < 1.2e-9)[2])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 1e-9 & dt$Kd_APN < 1.2e-9)[1])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 1e-9 & dt$Kd_APN < 1.2e-9)[2])
```
...
par(mfrow=c(2,2))
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 1e-10 & dt$Kd_APN < 1.2e-10)[1])
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 1e-10 & dt$Kd_APN < 1.2e-10)[2])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 1e-10 & dt$Kd_APN < 1.2e-10)[1])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 1e-10 & dt$Kd_APN < 1.2e-10)[2])
```
...
par(mfrow=c(2,2))
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 1e-11 & dt$Kd_APN < 2e-11)[1])
plot.titration(which(dt$library=="pool1A" & dt$Kd_APN > 1e-11 & dt$Kd_APN < 2e-11)[2])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 1e-11 & dt$Kd_APN < 2e-11)[1])
plot.titration(which(dt$library=="pool2A" & dt$Kd_APN > 1e-11 & dt$Kd_APN < 2e-11)[2])
```
...
median.nMSR <- median(dt$nMSR_APN,na.rm=T)
threshold <- 40
par(mfrow=c(2,2))
for(bg in c("Wuhan_Hu_1","BA1","BA2")){
  plot(log10(dt[target==bg,TiteSeq_avgcount]),dt[target==bg,nMSR_APN],main=bg,pch=19,col="#00000010",xlab="average cell count (log10)",ylab="nMSR",xlim=c(0,6),ylim=c(0,0.5))
  abline(h=threshold*median.nMSR,col="red",lty=2)
  legend("topleft",bty="n",cex=1,legend=paste(format(100*nrow(dt[target==bg & nMSR_APN > threshold*median.nMSR & !is.na(nMSR_APN),])/nrow(dt[target==bg & !is.na(nMSR_APN),]),digits=3),"%"))
}

dt[nMSR_APN > threshold*median.nMSR,c("Kd_APN","Kd_SE_APN","response_APN","baseline_APN") := list(as.numeric(NA),as.numeric(NA),as.numeric(NA),as.numeric(NA))]
...
```{r convert_log10Ka}
dt[,log10Ka := -log10(Kd_APN),by=c("barcode","library")]
```
