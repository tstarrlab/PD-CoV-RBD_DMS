---
title: "Compute per-barcode pAPN binding"
author: "Tyler Starr"
date: "12/30/2022"
output:
  github_document:
...
---
This notebook reads in per-barcode counts from `count_variants.ipynb` for pAPN binding Sort-seq experiments, computes functional scores for pAPN binding levels, and does some basic QC on variant pAPN functional scores.
...
Let's look at the distibution of pAPN scores by variant class for each library.

```{r unfiltered_pAPN_meanF_distribution, echo=T, fig.width=7, fig.height=5, fig.align="center", dpi=300,dev="png"}
...
Do as violin plots, faceted by each target. In next notebook, we'll evaluate count depth and possibly apply further filtering to remove low-count pAPN_meanF estimates

```{r pAPN_meanF_distribution_vioplot, echo=T, fig.width=6, fig.height=18, fig.align="center", dpi=300,dev="png"}
...
We have generated pAPN_meanF measurements for `r round(nrow(dt[!is.na(pAPN_meanF)])/nrow(dt)*100,digits=2)`% of the barcodes in our libraries.
