---
title: "Compute per-barcode hAPN binding"
author: "Tyler Starr"
date: "12/30/2022"
output:
  github_document:
...
---
This notebook reads in per-barcode counts from `count_variants.ipynb` for hAPN binding Sort-seq experiments, computes functional scores for hAPN binding levels, and does some basic QC on variant hAPN functional scores.
...
Let's look at the distibution of hAPN scores by variant class for each library.

```{r unfiltered_hAPN_meanF_distribution, echo=T, fig.width=7, fig.height=5, fig.align="center", dpi=300,dev="png"}
...
Do as violin plots, faceted by each target. In next notebook, we'll evaluate count depth and possibly apply further filtering to remove low-count hAPN_meanF estimates

```{r hAPN_meanF_distribution_vioplot, echo=T, fig.width=6, fig.height=18, fig.align="center", dpi=300,dev="png"}
...
We have generated hAPN_meanF measurements for `r round(nrow(dt[!is.na(hAPN_meanF)])/nrow(dt)*100,digits=2)`% of the barcodes in our libraries.
